...
const MapComponent: React.FC<MapComponentProps> = ({ selectedProvince, onProvinceClick }) => {
  ...
  // قائمة جميع الأقاليم المتاحة محلياً للبحث السريع
  ...
  }, [onProvinceClick, getProvinceName, zoomToProvince]);
  ...
  const handleSearch = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const arabicMatches = Object.entries(ARABIC_PROVINCES)
      .filter(([ar, fr]) => ar.includes(q) || ar.includes(normalizedQ))
      .map(([ar, fr]) => ({
        display_name: fr,
        type: 'local',
        province: fr
    ...
    const combinedLocal = Array.from(new Map([...localMatches, ...arabicMatches].map(item => [item.province, item])).values());
          ...
          <div className="relative animate-in slide-in-from-right-4 flex flex-col items-end">
            <div className="relative">
              <input 
                ...
                placeholder="Rechercher (Ex: Rabat, أكادير...)" 
