...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    <>
      <aside className={`hidden md:flex flex-col w-72 bg-white border-r border-slate-200 h-full transition-all duration-300 shadow-xl z-30`}>
      ...
      {isMobileDrawerOpen && (
        <div 
          className="md:hidden fixed inset-0 bg-slate-900/40 backdrop-blur-[2px] z-[2000]"
          onClick={() => setIsMobileDrawerOpen(false)}
        >
          <div 
            className="absolute top-0 left-0 h-full w-[80%] max-w-[300px] bg-white shadow-2xl flex flex-col"
          ...
          >
            ...
            <div className="p-3 space-y-2 bg-white border-b border-slate-50">
              <input
                ...
                className="w-full pl-3 pr-3 py-2 bg-slate-100 border-none rounded-xl text-[10px] font-bold outline-none"
              />
            </div>
            <div className="flex-1 overflow-y-auto px-1 py-2">
              {provinces.map((p) => (
                <button
                  ...
                  className={`w-full flex items-center px-3 py-2 text-left rounded-lg transition-all ${selectedProvince === p.nomFr ? 'bg-blue-600 text-white' : 'bg-white'}`}
                >
                  <div className="flex-1 min-w-0">
                    <div className={`text-[10px] font-black uppercase truncate ${selectedProvince === p.nomFr ? 'text-white' : 'text-slate-800'}`}>{p.nomFr}</div>
                  </div>
                </button>
              ))}
            </div>
