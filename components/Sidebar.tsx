...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    <>
      ...
      {isMobileDrawerOpen && (
        <div className="md:hidden fixed inset-0 bg-slate-900/40 backdrop-blur-[2px] z-[2000]" onClick={() => setIsMobileDrawerOpen(false)}>
          <div className="absolute top-0 left-0 h-full w-[80%] max-w-[300px] bg-white shadow-2xl flex flex-col" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex-1 overflow-y-auto px-1 py-2">
              {provinces.map((p) => (
                <button
                  ...
                  className={`w-full flex items-center px-3 py-2 text-left rounded-lg transition-all ${selectedProvince === p.nomFr ? 'bg-blue-600 text-white shadow-md' : 'bg-white'}`}
                >
